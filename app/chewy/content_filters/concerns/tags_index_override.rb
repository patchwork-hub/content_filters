module ContentFilters
  module Concerns
    module TagsIndexOverride
      extend ActiveSupport::Concern
      
      def self.prepended(base)
        base.class_eval do
          # Preserve existing settings and crutch
          settings index: index_preset(refresh_interval: '30s'), analysis: {
            analyzer: {
              content: {
                tokenizer: 'keyword',
                filter: %w(word_delimiter_graph lowercase asciifolding cjk_width),
              },
              edge_ngram: {
          ...
          # Redefine index_scope
          index_scope ::Tag.listable.without_banned

          # Redefine root with additional is_banned field
